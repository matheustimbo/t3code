...
## Subscription usage limits beyond Claude and Codex

Claude and Codex publish `usageLimits` from their own probes and rate-limit
events. Cursor, Grok, and OpenCode Go have no runtime channel, so each driver
runs a polling loop from `providerUsageLimitPolling.ts` beside its status probe,
gated by `BackgroundPolicy` so an idle app is not hitting a billing endpoint every
45 seconds. Their payload parsers live in `Layers/polledUsageLimits.ts`. A read
that fails keeps the last good bars: `resolveUsageLimitsAfterProbe` treats
`probeFailed` as no new information.

A CLIProxyAPI hub reports no usage percentages itself, so `usage/cliproxyUsageLimits.ts`
enumerates the pooled credentials through the management API's `auth-files` route
and replays each provider's own usage request through `api-call`. Both payloads are
reshaped into what the native drivers produce and handed to the same mappers, so a
pooled row and a local row stay the same row.
