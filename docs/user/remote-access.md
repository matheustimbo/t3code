...
`tar`, and `sha256sum` or `shasum`. It does not need Node.js for the T3 Code
server. Install and authenticate each [provider](./install.md#providers) on the
remote host. The first launch downloads T3 Code's server to `~/.t3/runtime`
there, so it takes longer than later ones.
