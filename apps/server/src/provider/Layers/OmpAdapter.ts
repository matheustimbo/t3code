...
import { OmpTransportFailure } from "../acp/OmpTransportFailure.ts";
import { type OmpAdapterShape } from "../Services/OmpAdapter.ts";
import { resolveOmpAcpBaseModelId } from "./OmpProvider.ts";
...
interface OmpSessionContext {
  ...
  assistantReply: OmpTransportFailure;
  stopped: boolean;
}
...
export function makeOmpAdapter(ompSettings: OmpSettings, options?: OmpAdapterLiveOptions) {
  return Effect.gen(function* () {
        ...
        Effect.mapError(
          (cause) =>
            new EffectAcpErrors.AcpTransportError({
              detail: "Failed to process omp ACP permission event.",
              cause,
            }),
        ),
      );
    ...
    const logNative = (
      ...
      method: string,
      payload: unknown,
      _source: "acp.jsonrpc" | "acp.omp.extension",
    ) =>
      Effect.gen(function* () {
    ...
    const emitPlanUpdate = (
      ...
      rawPayload: unknown,
      source: "acp.jsonrpc" | "acp.omp.extension",
      method: string,
    ) =>
      Effect.gen(function* () {
      ...
      withThreadLock(
        ...
        Effect.gen(function* () {
          ...
          ctx = {
            ...
            promptsInFlight: 0,
            assistantReply: new OmpTransportFailure(),
            stopped: false,
          };
              ...
              Effect.gen(function* () {
                switch (event._tag) {
                  ...
                  case "AssistantItemStarted":
                    ctx.assistantReply = new OmpTransportFailure();
                    yield* offerRuntimeEvent(
                      makeAcpAssistantItemEvent({
                        stamp: yield* makeEventStamp(),
                        provider: PROVIDER,
        ...
        return yield* Effect.gen(function* () {
          ...
          if (input.attachments && input.attachments.length > 0) {
            for (const attachment of input.attachments) {
              // omp ingests images (see promptCapabilities). Generic files reach the agent
            ...
            .prompt({
              prompt: /^\/[^\s/]+(?:\s|$)/.test(rawPrompt)
                ...
                : [
                    ...
                    {
                      type: "text",
                      text: buildRuntimeInstructions({ harness: "Omp", model: resolvedModel }),
                    },
                  ],
            })
            .pipe(
          ...
          if (ctx.promptsInFlight === 1 && result.stopReason !== "cancelled" && failure) {
            return yield* new ProviderAdapterRequestError({
              provider: PROVIDER,
              method: "session/prompt",
              detail: "omp reported a transport failure.",
              cause: failure,
            });
          }
    ...
    const rollbackThread: OmpAdapterShape["rollbackThread"] = (threadId, numTurns) =>
      Effect.gen(function* () {
        ...
        return yield* new ProviderAdapterRequestError({
          ...
          detail: "omp ACP sessions do not support provider-side rollback.",
        });
      });
