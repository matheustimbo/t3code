  ...
  create: ({ instanceId, displayName, accentColor, environment, enabled, config }) =>
    Effect.gen(function* () {
      ...
      const snapshot = yield* makeManagedServerProvider<ProviderSnapshotSettings<OmpSettings>>({
        ...
        checkProvider,
        enrichSnapshot: ({ settings, getSnapshot, publishSnapshot, backgroundPolicy }) =>
          Effect.gen(function* () {
            void backgroundPolicy;
            yield* enrichOmpSnapshot({
            ...
            // omp exposes no usage-limits API today; the snapshot always
            // reports limits as unavailable so the UI never shows stale data.
            ...
            yield* publishSnapshot({
              ...
              usageLimits: makeUnavailableUsageLimits({
                ...
                reason: "unsupported",
                message: settings.provider.usageLimitsEnabled
                  ? "omp plan limits are not readable through the CLI yet."
                  : "Experimental omp plan limits are disabled in provider settings.",
              }),
            });
          }),
      }).pipe(
